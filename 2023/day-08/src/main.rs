...
fn process_input(input: &str) -> (Vec<usize>, HashMap<&str, (&str, &str)>) {
    let blocks = input.split_once("\n\n").unwrap();

    let instructions: Vec<usize> = blocks.0.chars().filter_map(|char| {
        match char {
            'L' => Some(0),
            'R' => Some(1),
            _ => None
    ...
    let map: HashMap<_, _> = re.captures_iter(blocks.1).map(|caps| {
...
fn part1(instructions_map: &(Vec<usize>, HashMap<&str, (&str, &str)>)) -> u64 {
    ...
    instructions.iter().cycle().enumerate().find_map(|(i, instruction)| {
...
fn part2(instructions_map: &(Vec<usize>, HashMap<&str, (&str, &str)>)) -> u64 {
    ...
    let starting_nodes: Vec<&str> = map.keys()
    ...
    starting_nodes.iter().map(|starting_node| {
