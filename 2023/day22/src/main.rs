...
impl Solution for Day22 {
    ...
    fn process(input: &str) -> Self::Input<'_> {
        let re = Regex::new(r"(?<x0>\d+),(?<y0>\d+),(?<z0>\d+)~(?<x1>\d+),(?<y1>\d+),(?<z1>\d+)")
            .unwrap();
        ...
        let max_xyz = bricks.iter().fold((0, 0, 0), |acc, brick| {
            (
    ...
    fn part1((supporting, supported_by): &Self::Input<'_>) -> Self::Output1 {
        supporting
            .keys()
            .par_bridge()
            .filter(|id| {
    ...
    fn part2((supporting, supported_by): &Self::Input<'_>) -> Self::Output2 {
        supporting
            .keys()
            .par_bridge()
            .map(|id| {
...
mod tests {
    ...
    aoc_utils::solution_tests!(2023, 22, Day22);

    #[rstest]
    fn part1_test(data1: <Day22 as Solution>::Input<'_>) {
        assert_eq!(5, Day22::part1(&data1));
    }

    #[rstest]
    fn part2_test(data1: <Day22 as Solution>::Input<'_>) {
        assert_eq!(7, Day22::part2(&data1));
    }
}
