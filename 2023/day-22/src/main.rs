...
impl Brick {
    fn from(id: usize, x0: usize, y0: usize, z0: usize, x1: usize, y1: usize, z1: usize) -> Self {
        let bottom = (x0..=x1).cartesian_product(y0..=y1)
            .map(|(x, y)| (x, y, z0))
            .collect();
        let top = (x0..=x1).cartesian_product(y0..=y1)
            .map(|(x, y)| (x, y, z1))
            .collect();
        Brick { id, min: (x0, y0, z0), max: (x1, y1, z1), bottom, top }
    }
    ...
    fn contains(&self, coord: &(usize, usize, usize)) -> bool {
        coord.0 >= self.min.0 && coord.0 <= self.max.0
            ...
            && coord.2 >= self.min.2 && coord.2 <= self.max.2
    }
}
...
fn print_stack_side(bricks_max: &(Vec<Brick>, (usize, usize, usize)), looking_in_y_dir: bool) {
    ...
    for z in (1..=max_xyz.2).rev() {
        for s in &side {
            let bricks_here: Vec<usize> = depth.iter().filter_map(|d| {
                let coord = to_coord(z, *s, *d);
                bricks.iter().find_map(|brick| brick.contains(&coord).then_some(brick.id))
            }).unique().collect();
...
type ProcessedData = (HashMap<usize, HashSet<usize>>, HashMap<usize, HashSet<usize>>);

fn process_input(input: &str) -> ProcessedData {
    let re = Regex::new(r"(?<x0>\d+),(?<y0>\d+),(?<z0>\d+)~(?<x1>\d+),(?<y1>\d+),(?<z1>\d+)").unwrap();
    let mut bricks: Vec<Brick> = re.captures_iter(input).enumerate().map(|(id, caps)| {
    ...
    let mut max_xyz = bricks.iter().fold((0, 0, 0), |acc, brick| {
        (acc.0.max(brick.max.0), acc.1.max(brick.max.1), acc.2.max(brick.max.2))
    });
    ...
    for brick in &mut bricks {
        let new_bottom_z = brick.bottom.iter().map(|(x, y, _z)| next_valid[*y][*x]).max().unwrap();
    ...
    let supporting: HashMap<usize, HashSet<usize>> = bricks.par_iter()
        .map(|brick| {
            let z = brick.max.2 + 1;
            let supported = bricks.iter()
                .filter_map(|other| {
                    (brick.id != other.id
                        && other.min.2 == z
                        && brick.min.0 <= other.max.0 && other.min.0 <= brick.max.0
                        && brick.min.1 <= other.max.1 && other.min.1 <= brick.max.1)
                        .then_some(other.id)
                }).collect();
            (brick.id, supported)
        })
    ...
    let supported_by: HashMap<usize, HashSet<usize>> = bricks.par_iter()
        .map(|brick| {
            let z = brick.min.2 - 1;
            let supported = bricks.iter()
                .filter_map(|other| {
    ...
    supporting.keys().par_bridge()
        .map(|id| {
            ...
            while let Some(id) = check_next.pop() {
                let will_fall: Vec<usize> = supporting[&id].iter()
