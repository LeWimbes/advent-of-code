...
fn process_input(input: &str) -> (Vec<u64>, Vec<Mapping>) {
    let mut blocks_it = input.split("\n\n");
    let seeds: Vec<u64> = blocks_it.next().unwrap()
        .split_once(":").unwrap().1
        .split_whitespace()
        .map(|num| num.parse().unwrap())
        .collect();

    let mappings: Vec<Mapping> = blocks_it.map(|block| {
        let mappings = block.lines().skip(1).map(|mapping| {
            let nums: Vec<u64> = mapping.split_whitespace().map(|num| num.parse().unwrap()).collect();
...
fn part1(seeds_mappings: &(Vec<u64>, Vec<Mapping>)) -> u64 {
    seeds_mappings.0.iter()
        .map(|seed| {
            seeds_mappings.1.iter()
                .fold(*seed, |value, mapping| mapping.map(value))
        }).min().unwrap()
}

fn part2(seeds_mappings: &(Vec<u64>, Vec<Mapping>)) -> u64 {
    seeds_mappings.0.par_chunks_exact(2).flat_map(|chunk| {
