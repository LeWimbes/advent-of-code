...
fn process_input(input: &str) -> HashMap<(usize, usize), Vec<u32>> {
            ...
            for r in row.saturating_sub(1)..(row + 2).min(grid.len()) {
                for c in m.start().saturating_sub(1)..(m.end() + 1).min(grid[row].len()) {
                    if !non_chars.contains(&grid[r][c]) {
                        let key = (r, c);
                        if chars.contains_key(&key) {
                            chars.get_mut(&key).unwrap().push(value);
                        } else {
                            chars.insert(key, Vec::from([value]));
                        }
                    }
                }
            }
...
fn part1(chars: &HashMap<(usize, usize), Vec<u32>>) -> u32 {
    chars.iter().map(|(_key, value)| value.iter().sum::<u32>()).sum()
}

fn part2(chars: &HashMap<(usize, usize), Vec<u32>>) -> u32 {
    chars.iter().filter_map(|(_key, value)| {
        if value.len() == 2 {
