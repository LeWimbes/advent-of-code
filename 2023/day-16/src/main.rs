...
impl Direction {
    fn get_next_coord(self, coord: &(usize, usize), bounds: &(usize, usize)) -> Option<(usize, usize)> {
        match self {
            Direction::North => if coord.1 != 0 {
                return Some((coord.0, coord.1 - 1));
            },
            Direction::West => if coord.0 != 0 {
                return Some((coord.0 - 1, coord.1));
            },
            Direction::South => if coord.1 < bounds.1 - 1 {
                return Some((coord.0, coord.1 + 1));
            },
            Direction::East => if coord.0 < bounds.0 - 1 {
                return Some((coord.0 + 1, coord.1));
            },
        };
        return None;
    ...
    fn get_next_ray(self, coord: &(usize, usize), bounds: &(usize, usize)) -> Option<Ray> {
        self.get_next_coord(coord, bounds).map(|new_coord| Ray::from(new_coord, self))
    }

    fn get_next_rays(self, coord: &(usize, usize), bounds: &(usize, usize)) -> Vec<Ray> {
        self.get_next_ray(coord, bounds).map(|next_ray| vec![next_ray]).unwrap_or(vec![])
    }
}
...
impl Ray {
    ...
    fn get_next_ray(self, bounds: &(usize, usize)) -> Option<Ray> {
        if let Some(new_coord) = self.direction.get_next_coord(&self.coord, bounds) {
            Some(Ray::from(new_coord, self.direction))
        } else {
            None
        }
    }
    ...
    fn get_next_rays(self, bounds: &(usize, usize)) -> Vec<Ray> {
        self.get_next_ray(bounds).map(|new_ray| vec![new_ray]).unwrap_or(vec![])
    }
}

fn get_next_rays(directions: Vec<Direction>, coord: &(usize, usize), bounds: &(usize, usize)) -> Vec<Ray> {
    directions.iter()
        .filter_map(|direction| direction.get_next_coord(coord, bounds)
    ...
    fn get_outgoing_rays(self, incoming: &Ray, bounds: &(usize, usize)) -> Vec<Ray> {
        match self {
            ...
            TileType::Vertical => match incoming.direction {
                ...
                Direction::West | Direction::East => get_next_rays(vec![Direction::North, Direction::South], &incoming.coord, bounds),
            }

            TileType::Horizontal => match incoming.direction {
                Direction::North | Direction::South => get_next_rays(vec![Direction::West, Direction::East], &incoming.coord, bounds),
...
fn process_input(input: &str) -> Vec<Vec<TileType>> {
    input.lines().map(|row| {
        row.chars().map(|char| match char {
            '/' => TileType::Slash,
            '\\' => TileType::Backslash,
            '|' => TileType::Vertical,
            '-' => TileType::Horizontal,
...
fn get_energized_tiles(layout: &Vec<Vec<TileType>>, start: Ray) -> u64 {
...
fn part1(layout: &Vec<Vec<TileType>>) -> u64 {
    get_energized_tiles(layout, Ray::from((0, 0), Direction::East))
}

fn part2(layout: &Vec<Vec<TileType>>) -> u64 {
    // top
    (0..layout[0].len()).zip(repeat_n(0, layout.len())).zip(repeat_n(Direction::South, layout[0].len()))
        // bottom
        .chain((0..layout[0].len()).zip(repeat_n(0, layout.len())).zip(repeat_n(Direction::North, layout[0].len())))
        // left
        .chain(repeat_n(0, layout[0].len()).zip(0..layout.len()).zip(repeat_n(Direction::East, layout[0].len())))
        // right
        .chain(repeat_n(0, layout[0].len()).zip(0..layout.len()).zip(repeat_n(Direction::West, layout[0].len())))
