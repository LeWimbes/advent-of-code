...
impl Solution for Day19 {
    type Input<'a> = (Vec<PartRange>, Vec<Part>);
    type Output1 = u64;
    type Output2 = u64;
    ...
    fn part1((accepted, parts): &Self::Input<'_>) -> Self::Output1 {
        parts
            .iter()
    ...
    fn part2((accepted, _parts): &Self::Input<'_>) -> Self::Output2 {
        accepted.iter().map(PartRange::combinations).sum()
    }
}

aoc_utils::run!(2023, 19, Day19);
...
impl Rule {
    fn from(
        ...
        if let Some(cat) = category
            && let Some(comp) = comparator
            && let Some(val) = value
        {
            let category = cat.try_into().expect("Unknown category!");
            let comparator = comp.try_into().expect("Unknown comparator!");
            let value = val.parse().expect("Not an u64!");
...
mod tests {
    ...
    aoc_utils::solution_tests!(2023, 19, Day19);

    #[rstest]
    fn part1_test(data1: <Day19 as Solution>::Input<'_>) {
        assert_eq!(19_114, Day19::part1(&data1));
    }

    #[rstest]
    fn part2_test(data1: <Day19 as Solution>::Input<'_>) {
        assert_eq!(167_409_079_868_000, Day19::part2(&data1));
    }
}
