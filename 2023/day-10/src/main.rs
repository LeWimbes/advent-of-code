...
fn push_top_if_valid(area: &Vec<Vec<TileType>>, coord: (usize, usize), neighbors: &mut Vec<(usize, usize)>) {
    let (x, y) = coord;
    if y > 0 {
        let top = area[y - 1][x];
        if top == TileType::Start || top == TileType::TopBottom || top == TileType::BottomLeft || top == TileType::BottomRight {
            neighbors.push((x, y - 1));
        }
    }
}

fn push_bottom_if_valid(area: &Vec<Vec<TileType>>, coord: (usize, usize), neighbors: &mut Vec<(usize, usize)>) {
    let (x, y) = coord;
    if y < area.len() - 1 {
        let bottom = area[y + 1][x];
        if bottom == TileType::Start || bottom == TileType::TopBottom || bottom == TileType::TopRight || bottom == TileType::TopLeft {
            neighbors.push((x, y + 1));
        }
    }
}

fn push_left_if_valid(area: &Vec<Vec<TileType>>, coord: (usize, usize), neighbors: &mut Vec<(usize, usize)>) {
    let (x, y) = coord;
    if x > 0 {
        let left = area[y][x - 1];
        if left == TileType::Start || left == TileType::LeftRight || left == TileType::TopRight || left == TileType::BottomRight {
            neighbors.push((x - 1, y));
        }
    }
}

fn push_right_if_valid(area: &Vec<Vec<TileType>>, coord: (usize, usize), neighbors: &mut Vec<(usize, usize)>) {
    let (x, y) = coord;
    if x < area[y].len() - 1 {
        let right = area[y][x + 1];
        if right == TileType::Start || right == TileType::LeftRight || right == TileType::TopLeft || right == TileType::BottomLeft {
            neighbors.push((x + 1, y));
        }
    }
}

fn neighbors(area: &Vec<Vec<TileType>>, coord: (usize, usize)) -> Vec<(usize, usize)> {
...
fn pretty_print_area(area: &Vec<Vec<TileType>>, colors: &Vec<(Color, HashSet<&(usize, usize)>)>) {
    ...
    area.iter().enumerate().for_each(|(y, row)| {
        let row_str = row.iter().enumerate().map(|(x, tile)| {
            let coord = (x, y);
...
fn find_cycle(area: &Vec<Vec<TileType>>, start: &(usize, usize)) -> Option<Vec<(usize, usize)>> {
    ...
    while let Some((current, parent)) = stack.pop_back() {
        ...
        neighbors(area, current).iter().for_each(|child| {
            if child != &parent {
                parent_map.insert(child.clone(), current);
                stack.push_back((child.clone(), current));
            }
        });
    }
...
fn process_input(input: &str) -> (Vec<Vec<TileType>>, (usize, usize), Vec<(usize, usize)>) {
    let mut start = None;
    let mut area: Vec<Vec<TileType>> = input.lines().enumerate()
        .map(|(y, line)| {
    ...
    let cycle = [TileType::TopBottom,
        TileType::LeftRight,
        TileType::TopRight,
        TileType::TopLeft,
        TileType::BottomLeft,
        TileType::BottomRight].iter()
        .find_map(|tile_type| {
...
fn part2(area_start_cycle: &(Vec<Vec<TileType>>, (usize, usize), Vec<(usize, usize)>)) -> u64 {
    let (area, start, cycle) = area_start_cycle;
    let cycle: HashSet<_> = cycle.iter().collect();
    let clean_area: Vec<Vec<TileType>> = area.iter().enumerate()
        .map(|(y, row)| {
            row.iter().enumerate()
                .map(|(x, tile)| {
    ...
    let enclosed_tiles: HashSet<(usize, usize)> = clean_area.iter().enumerate().flat_map(|(y, row)| {
        let mut inside = false;
        let mut last_changer = TileType::TopBottom;
        row.iter().enumerate().filter_map(|(x, tile)| {
            let coord = (x, y);
    ...
    let colors = vec![(Color::Red, HashSet::from([start])), (Color::Blue, cycle), (Color::Yellow, enclosed_tiles.iter().collect())];
