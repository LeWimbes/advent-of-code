...
fn process_input(input: &str) -> (Vec<(i64, i64)>, Vec<(i64, i64)>) {
    let re = Regex::new(r"(?<direction1>[UDLR]) (?<meters1>\d+) \(#(?<meters2>[0-9a-fA-F]{5})(?<direction2>\d)\)").unwrap();
    let instructions: Vec<Instruction> = re.captures_iter(input).map(|caps| {
        let direction1 = caps["direction1"].chars().next().unwrap().try_into().unwrap();
        let meters1 = caps["meters1"].parse().unwrap();
        let direction2 = caps["direction2"].chars().next().unwrap().try_into().unwrap();
        let meters2 = i64::from_str_radix(&caps["meters2"], 16).unwrap();
        Instruction { direction1, meters1, direction2, meters2 }
    }).collect();
...
fn calculate_area(coords: &[(i64, i64)]) -> i64 {
    ...
    let b = coord_pairs.iter()
        .map(|((x1, y1), (x2, y2))| (x1 - x2).abs() + (y1 - y2).abs()).sum::<i64>();

    let i = coord_pairs.iter()
        .map(|((x1, y1), (x2, y2))| x1 * y2 - x2 * y1).sum::<i64>().abs() / 2;
