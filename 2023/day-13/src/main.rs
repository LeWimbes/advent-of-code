...
fn process_input(input: &str) -> Vec<Vec<Vec<bool>>> {
    input.split("\n\n").map(|pattern| {
        pattern.lines()
            .map(|line| line.chars().map(|tile| {
                match tile {
                    '.' => false,
                    '#' => true,
...
fn find_reflections(patterns: &Vec<Vec<Vec<bool>>>, smudges: usize) -> u64 {
    let (columns, rows) = patterns.iter().fold((0, 0), |(columns, rows), pattern| {
        let rows_above = (1..pattern.len()).find(|rows_above| {
            ...
            above_range.rev().zip(below_range)
                .map(|(above, below)| pattern[above].iter().zip(pattern[below].iter())
        ...
        let cols_left = (1..pattern[0].len()).find(|cols_left| {
            ...
            left_range.rev().zip(right_range)
                .map(|(left, right)| pattern.iter()
...
fn part1(patterns: &Vec<Vec<Vec<bool>>>) -> u64 {
    find_reflections(patterns, 0)
}

fn part2(patterns: &Vec<Vec<Vec<bool>>>) -> u64 {
