...
fn part2(instructions: &Vec<&str>) -> u64 {
    ...
    let instructions: Vec<_> = instructions.iter().map(|instruction| {
    ...
    instructions.iter().for_each(|instruction| match instruction {
        Instruction::Remove(box_index, label) => {
            let index = boxes[*box_index].iter().position(|lens| &lens.label == label);
    ...
    boxes.iter().enumerate().map(|(box_index, lens_box)| {
        let box_part = (box_index + 1) as u64;
        lens_box.iter().enumerate().map(|(lens_index, lens)| {
            (lens_index + 1) as u64 * lens.focal_length as u64
        }).sum::<u64>() * box_part
    }).sum::<u64>()
}
