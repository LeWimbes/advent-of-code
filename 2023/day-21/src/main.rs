...
fn process_input(input: &str) -> (Vec<Vec<bool>>, (isize, isize)) {
    let mut start = (0, 0);
    let map = input
        .lines()
        .enumerate()
...
fn get_neighbors(coord: &(isize, isize), map: &Vec<Vec<bool>>) -> Vec<(isize, isize)> {
    let upper_bounds = (map[0].len() as isize, map.len() as isize);
    let changes_x = [0, -1, 0, 1];
    let changes_y = [-1, 0, 1, 0];

    changes_x.iter().zip(changes_y).filter_map(|(x_change, y_change)| {
...
fn count_reachable_tiles_quadratic(
    ...
    let result = (a0 + a1 * n as f64 + a2 * (n * n) as f64) as usize;
...
fn fit_quadratic_polynomial(x0: f64, y0: f64, x1: f64, y1: f64, x2: f64, y2: f64) -> (f64, f64, f64) {
    let v = Matrix3::new(1.0, x0, x0 * x0,
                         1.0, x1, x1 * x1,
                         1.0, x2, x2 * x2);
...
fn part2(map_start: &(Vec<Vec<bool>>, (isize, isize))) -> usize {
    count_reachable_tiles_quadratic(map_start, 26501365)
}
...
mod tests {
    ...
    fn part2_test5(data: (Vec<Vec<bool>>, (isize, isize))) {
        assert_eq!(count_reachable_tiles_quadratic(&data, 500), 167004);
    }

    #[rstest]
    fn part2_test6(data: (Vec<Vec<bool>>, (isize, isize))) {
        assert_eq!(count_reachable_tiles_quadratic(&data, 1000), 668697);
    }

    #[rstest]
    fn part2_test7(data: (Vec<Vec<bool>>, (isize, isize))) {
        assert_eq!(count_reachable_tiles_quadratic(&data, 5000), 16733044);
    }
}
