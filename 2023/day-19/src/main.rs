...
impl PartRange {
    ...
    fn replace(&self, category: Category, range: Range<u64>) -> Self {
        match category {
            Category::X => PartRange { x: range, m: self.m.clone(), a: self.a.clone(), s: self.s.clone() },
            Category::M => PartRange { x: self.x.clone(), m: range, a: self.a.clone(), s: self.s.clone() },
            Category::A => PartRange { x: self.x.clone(), m: self.m.clone(), a: range, s: self.s.clone() },
            Category::S => PartRange { x: self.x.clone(), m: self.m.clone(), a: self.a.clone(), s: range },
        }
    }

    fn empty() -> Self {
        PartRange { x: 0..0, m: 0..0, a: 0..0, s: 0..0 }
    }
...
impl Rule {
    fn from(category: Option<&str>, comparator: Option<&str>, value: Option<&str>, result: &str) -> Self {
    ...
    fn apply(&self, range: &PartRange) -> ((&ResultType, PartRange), PartRange) {
        match self {
            Rule::Complex(cat, comp, val, res) => {
                ...
                ((res, range.replace(*cat, parts.0)), range.replace(*cat, parts.1))
            }
...
fn compile(workflows: &HashMap<String, Workflow>) -> Vec<PartRange> {
    ...
    while let Some((label, range)) = remaining.pop() {
        ...
        accepted.extend(result.0.iter().filter_map(|range| (range.combinations() != 0).then_some(range.clone())));
        remaining.extend(result.1.iter().filter_map(|(label, range)| (range.combinations() != 0).then_some(((*label).clone(), range.clone()))));
    }

    accepted
}

fn process_input(input: &str) -> (Vec<PartRange>, Vec<Part>) {
    ...
    let workflows: HashMap<String, Workflow> = re_workflows.captures_iter(input_parts.0).map(|caps| {
        let name = caps["name"].to_string();
        let rules: Vec<Rule> = caps["rules"].split(',').map(|rule| {
            let caps = re_rule.captures(rule).unwrap();
            Rule::from(caps.name("category").map(|m| m.as_str()),
                       caps.name("comparator").map(|m| m.as_str()),
                       caps.name("value").map(|m| m.as_str()),
                       caps.name("result").or(caps.name("simple")).map(|m| m.as_str()).unwrap())
        }).collect();
    ...
    let parts: Vec<Part> = re_parts.captures_iter(input_parts.1).map(|caps| {
        Part {
...
fn part1(accepted_parts: &(Vec<PartRange>, Vec<Part>)) -> u64 {
    ...
    parts.iter().filter(|part| {
        accepted.iter().any(|range| range.contains(part))
    }).map(Part::sum).sum()
}
