...
static DIRECTIONS: Lazy<Vec<Direction>> = Lazy::new(|| {
...
impl Direction {
    ...
    fn get_next_coord(self, coord: &(usize, usize), bounds: &(usize, usize)) -> Option<(usize, usize)> {
        match self {
            ...
            Direction::South => if coord.1 + 1 < bounds.1 {
                return Some((coord.0, coord.1 + 1));
            },
            Direction::East => if coord.0 + 1 < bounds.0 {
                return Some((coord.0 + 1, coord.1));
            },
        };
...
impl State {
    fn from(cost: u32, coord: (usize, usize), direction: Direction, moves_in_direction: usize) -> Self {
        State { cost, coord, direction, moves_in_direction }
    }
...
fn shortest_path(layout: &Vec<Vec<u32>>, min_moves: usize, max_moves: usize) -> u32 {
    let bounds = (layout[0].len(), layout.len());
